...
catsPath.reverse()
const schemaItems = catsPath.map(cat => {
  return { name: cat.name, alias: cat.alias }
})
// @ts-ignore
useBreadcrumbsSchema(forProduct ? [...schemaItems, { name: props.productName }] : schemaItems)
</script>
...
<template>
  <div class="">
    ...
    <div
      v-else
      class="mt-3 border-b border-gray-200"></div>
  </div>
</template>
