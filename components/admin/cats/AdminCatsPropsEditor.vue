...
const deleteItem = (i) => {
  showMessage('Подтвердите удаление', `<p>Параметр "${pGroup.value[i].name}" будет удален.</p><p>Продолжить?</p>`,
      'info', () => {
...
const handleOK = async () => {
  ...
  if (pGroup.value.some(item => item.name === '')) {
    showMessage('Действие отменено!', 'Не все имена параметров заполнены!', 'error')
    return
  }
