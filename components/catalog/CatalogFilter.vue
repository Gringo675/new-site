...
const resetButtonDisabled = computed(() =>
  props.filter.every((fGroup) => fGroup.values.every((value) => !value.active)),
);
...
<template>
  <div
    class="mt-10 mb-4 flex w-full flex-col rounded-xl max-md:mt-0 max-md:bg-gray-200 max-md:px-2"
  >
    <UButton
    ...
    >
      <div class="">
        ...
        >
          ...
          <div
            class="col-span-2 ml-auto text-indigo-500 italic @[16.5rem]:text-lg @md:col-span-1 @md:mx-auto"
          >
            Нужна консультация?
          </div>
          <UButton
            ...
            @click="
              showFeedback({
                ...
                description:
                  'Напишите требуемые параметры инструмента, и мы подберем для Вас подходящие варианты.',
              })
            "
          />
        </div>
