...
## seo && performance 
- прописать редиректы
- insert Yandex && Google scripts (+ change cookie's policy)

## redirects
- no 404 error in devtools network
- no 404 errors in log
- how to handle non-existing links? Find all and redirect to the '/'.

## optimization
- showMobileMenu, showCatsMenu - отложенная гидрация? (Не Lazy)
...
Если при проектировании админки учесть удаление кэша для обновляемых страниц, тогда в принципе и сам swr будет не нужен, и maxAge можно поставить бесконечный.
