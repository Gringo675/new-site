...
- подкатегория: несколько пропсов одного вида?
  Варианты:
- создание доп. объединяющих пропсов (например, Особенность: высокоточные)
- хранить несколько значений пропсов в одном поле таблицы. Сильно усложнит алгоритм поиска всех категорий для товара. И не по феншую. Но самый понятный вариант. А кеширование нивелирует дороговизну.
- определять относящиеся к товару категории в процессе создания/изменения товаров/категорий. Пусть алгоритм будет сложный, но запускаться будет единожды, а не при каждом посещении. Минус - императивный путь вместо декларативного. (Тогда здесь же вычислять и похожие товары)
- хранить пропсы для категорий в отдельной таблицы. Тоже усложнит поиск, но не так сильно. (Или сильно?)
- хранить в закодированной строке вида 17.0.0.38|74|45.0.0.0.0.0.0.
  Группы - и, в группе - или.
  В каждой группе значение либо должно входить, либо не должно быть другого значения. А по id этого не понять. Только делать дополнительный запрос и разбивать на группы.
  2 варианта создания подкатегорий:
  1. Как алгоритм выбора
  2. Максимальное разнообразие (сайдэффеект - одинаковые категории)
