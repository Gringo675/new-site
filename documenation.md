...
Добавил в prices&quantity.xlsx сохранение измененной информации в новую базу. 
Важно помнить, что при импорте товаров их ID (новый) до и после импорта могут не совпадать (если в старую базу были добавлены товары). Соответственно нужно обновлять связи, ссылающиеся на товары (например, если будет включено кеширование страниц, чистить кеш для обновления "Похожих товаров"). 
...
Со вторым тоже совсем ничего не понятно, поймал похожие проблемы и с entry.js. Подозреваю, что дело все же в системе кэширования, но по идее сервер все равно должен полностью перезапускаться после деплоя, что он не делает. Возможно при предыдущих экспериментах c index.mjs я неправильно размещал restart.txt (не в папке server), что и было причиной ошибки. Вернулся к index.mjs. Продолжаем наблюдать.
WARNING: похоже юзеров больше нельзя переносить со старой базы. При переносе может поменяться ид у существующего юзера, и при следующей авторизации он залогиниться под чужим аккаунтом.
...
Убрал функционал, как бесполезный. Если в будущем возникнет потребность закрывать сайт, можно это сделать через .htaccess.
...
Есть проблема с bingbot (только с ним), когда при spa-переходе на другую страницу сервер обрубает ему доступ к /api/getData/... и возвращает 403 ошибку. Все очень странно. Начальный html сервер отдает хорошо. Позволяет логировать ошибку через /api/setError. Пытался получить данные с клиента через /api/setText - опять 403. Причину не понял. AI говорит, что у хостера стоит хитрый фильтр, возможно. Вместо решения остановился на перезагрузке страницы при данной ошибке.
