# ToDo:
- копейки в цене
- создать реф для состояния корзины (товаров, сумма, с ндс)
- обратная связь - title / descr props
...
- не смог изменить цвет текста у активного путкта в верхнем меню. active-class не хочет принимать соответствующий класс (text-orange-300). Проверить поведение в следующих релизах.
