...
async function main() {
  ...
  console.log(`DEBUG: Raw input: ${JSON.stringify(input)}`)

  // Normalize literal '\n' strings to actual newlines for shell compatibility
  const normalizedInput = input.replace(/\\n/g, '\n')

  // Regex to match typical stack trace lines: "at name (url:line:col)" or "at url:line:col"
  const stackRegex = /\bat\s+(?:[\w\s$.()]+(?=\s*\())?\s*\(?([^)]+?):(\d+):(\d+)\s*\)?/g
  let match

  while ((match = stackRegex.exec(normalizedInput)) !== null) {
...
Processing: ${fullMatch}`)
  ...
  if (!normalizedInput.match(stackRegex)) {
