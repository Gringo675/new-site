...
test('test', async ({ page }) => {
  ...
  await expect(googlePage.getByText('Sign in with Google')).toBeVisible()
  ...
  await expect(vkPage.getByText('Sign in to «chelinstrument.ru»')).toBeVisible()
  await vkPage.close()

  await page.getByRole('button', { name: 'Войти через mail.ru' }).click()
  const mailruPage = await page.waitForEvent('popup')
  await expect(mailruPage.getByText('Sign in to the account')).toBeVisible()
